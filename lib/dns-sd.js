...
DNSSD.getSocket = function() {
  return new Promise((resolve) => {
    if (!this.socket) {
      ...
      this.socket.init(DNSSD_PORT, false, true);
      this.socket.asyncListen({
        onPacketReceived: function(aSocket, aMessage) {
          var packet = new DNSPacket(aMessage.rawData);
          switch (packet.flags.QR) {
            case DNSCodes.QUERY_RESPONSE_CODES.QUERY:
      ...
      this.socket.joinMulticast(DNSSD_MULTICAST_GROUP);
    }
